  ...
  , letterMatches =
    {
      ...
      x: function(counts, type, data)
          {
            if (!around(counts.count, 1120, 45)) return false;

            return matchByType(type, counts.count,
            {
              mine_closed : 1163, // test
              mine_open   : 1120,
              neutral     : 1077,
              their_open  : 1105,
              their_closed: 0 // test
      ...
      z: function(counts, type, data)
          {
            if (!around(counts.count, 1155, 45)) return false;
            if (counts.left - counts.right > 100) return false;
...
function processPositions(context, letters, callback)
{
  ...
  {
    _.forEach(letters, function(o, k)
    {
      letters[k].weight = weightList[o.type];
    });

    callback(null, _.sortBy(letters, function(l){ return l.weight * -1;}));
  }
}
